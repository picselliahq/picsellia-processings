...
input_annotations_dict, input_annotations_coco, output_annotations_dict = load_input_output_annotations(input_dataset_version)
...
augment = iaa.Sometimes(1, iaa.Rotate(rotate=(-parameters['rotation_limit'], parameters['rotation_limit'])))
...
for asset in assets_to_augment:
    for i in range(nb_augmented_imgs_per_img):
        ...
        augmented_image, augmented_bboxes, augmented_polygons = augment(image=asset_image, bounding_boxes=bboxes, polygons=polygons)
        
        if len(augmented_bboxes) > 0:
            corrected_bboxes = list(map(lambda box: box.clip_out_of_image((h,w)), augmented_bboxes))
            corrected_bboxes = list(map(pascal_voc_to_coco, corrected_bboxes))
        else:
            corrected_bboxes = []
            
        if len(augmented_polygons) > 0:
            corrected_polygons = list(map(lambda poly: poly.clip_out_of_image((h,w)), augmented_polygons))
            corrected_polygons = list(map(format_imgaug_poly_to_int_list, corrected_polygons))
        else:
            corrected_polygons = []
        
        augmented_img_filename = image_filename.split(".")[0] + '_' + format(i) + '.' + image_filename.split(".")[1]
        augmented_image = save_image(augmented_image, augmented_img_filename, augmented_dataset_path)
        
        output_annotations_dict = save_image_in_annotations_dict(augmented_img_filename, output_img_id, augmented_image.width, augmented_image.height, output_annotations_dict)
        
        output_annotations_dict, annotation_id = save_annotations_in_annotations_dict(labels, corrected_bboxes, corrected_polygons, output_img_id, annotation_id, output_annotations_dict)
...
for asset in assets_not_to_augment:
    ...
    output_annotations_dict = save_image_in_annotations_dict(image_filename, output_img_id, image.width, image.height, output_annotations_dict)
    ...
    output_annotations_dict, annotation_id = save_annotations_in_annotations_dict(labels, bboxes, polygons, output_img_id, annotation_id, output_annotations_dict)
...
add_data_job.wait_for_status(statuses=JobStatus.SUCCESS, blocking_time_increment=100., attempts=100)
output_dataset.import_annotations_coco_file(file_path=augmented_annotations_path, force_create_label=True, fail_on_asset_not_found=False)