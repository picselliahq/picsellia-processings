...
def increment_polygons_list(annotation, polygons_list):
    if len(annotation['segmentation'])>0:
        keypoints = [(annotation['segmentation'][0][i], annotation['segmentation'][0][i+1], i//2) for i in range(0, len(annotation['segmentation'][0]), 2)]
...
def save_augmented_image(augmented_image, augmented_asset_filename, augmented_dataset_path):
    augmented_asset_filepath = os.path.join(augmented_dataset_path, augmented_asset_filename)
...
def save_annotations_in_annotations_dict(asset, img_id, current_annot_id, annotations_dict):
