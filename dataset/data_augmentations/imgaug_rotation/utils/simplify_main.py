...
def upload_data_to_datalake(client, filepaths):
    ...
    data_list = datalake.upload_data(filepaths, tags=["augmented", "processing"], error_manager=error_manager)
    error_paths = [error.path for error in error_manager.errors]

    while len(error_paths) != 0:
        error_manager.clear()
        data_list.append(datalake.upload_data(error_paths, tags=["augmented", "processing"], error_manager=error_manager))
        error_paths = [error.path for error in error_manager.errors]
