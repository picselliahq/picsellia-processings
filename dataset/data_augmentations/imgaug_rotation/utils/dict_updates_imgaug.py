...
def increment_bboxes_list(annotation, image_width, image_height, bboxes_list):
    ...
    if len(annotation['bbox'])>0 :
        ...
        if corrected_box is not None :
            ...
            imgaug_box = BoundingBox(x1=box_formatted[0], y1=box_formatted[1], x2=box_formatted[2], y2=box_formatted[3], label=annotation['category_id'])
...
def increment_polygons_list(annotation, polygons_list):
    if len(annotation['segmentation'])>0:
        polygon = [(annotation['segmentation'][0][i], annotation['segmentation'][0][i+1]) for i in range(0, len(annotation['segmentation'][0]), 2)]
...
def save_image_in_annotations_dict(asset_filename, img_id, image_width, image_height, annotations_dict):
    dict_img = {}
...
def save_annotations_in_annotations_dict(labels, bboxes, polygons, img_id, current_annot_id, annotations_dict):
...
def get_or_create_dataset(client, dataset_name, dataset_description):
    try:
        ...
        dataset = client.create_dataset(name=dataset_name, description=dataset_description, private=True)
    return dataset

def get_or_create_dataset_version(dataset, dataset_version_name, dataset_version_description, dataset_type):
    try:
        dataset_version = dataset.get_version(version=dataset_version_name)
    except ResourceNotFoundError:
        dataset_version = dataset.create_version(version=dataset_version_name, description=dataset_version_description, type = dataset_type)
